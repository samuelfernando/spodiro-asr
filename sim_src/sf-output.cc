...
SFOutput::SFOutput(Simulator *simulator, char* source, const char* base_name, const char* output_path) {
	//cout << "Output constructor " << source << " " << base_name << " " << output_path << endl; 	
	this -> simulator = simulator;
	string date = simulator -> get_date_string();
	string day = simulator -> get_day_string();
	...
	if (!strcmp(source, "pa")) {
		...
		if (out_file.is_open() && chunk_lengths.is_open() && controls.is_open()) {
			simulator -> my_log("Successfully opened out files in " + out_str);
			//cerr << "Successfully opened out files in " << out_str << endl;
		}
		else {
			simulator -> my_log("Failed to open files in " + out_str);
		...
		str = out_str+"/"+day+"/chunks/"+base_str+".chunk.txt";
		chunk_lengths.open(str.c_str(),  std::fstream::out);
		str = out_str+"/"+day+"/controls/"+base_str+".control.txt";
		controls.open(str.c_str(),  std::fstream::out);
...
void SFOutput::write_cut_prev(int cut) {
	char cur_time[84];
	simulator -> get_timestamp(cur_time);
...
void SFOutput::write_readback_actual(MatrixIndexT size) {
	char cur_time[84];
	simulator -> get_timestamp(cur_time);
	controls << cur_time << " Read back actual " << size << endl;
}
