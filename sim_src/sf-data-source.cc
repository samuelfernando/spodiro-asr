...
WavDataSource::WavDataSource(string utt, Simulator* simulator) {
	
	//cout << "base name " << base_name << endl;
	
	
	
	//base_name = simulator->gen_config->base_name;
...
void WavDataSource::start_impl() {
	...
	//int32 chunk_length = int32(samp_freq * simulator->decode_config->chunk_length_secs);
...
PortAudioDataSource::PortAudioDataSource(Simulator* simulator, const char* output_path, const char* record_wav) {
...
void PortAudioDataSource::start_impl() {
	...
	while (active) {
		...
		simulator -> add_chunk(vec_data.Dim());
