...
SFNnetConfig::SFNnetConfig(int argc, char*argv[]) {
	...
	OnlineEndpointRule rule2(true, 0.2, 2.0, 0.0);
    OnlineEndpointRule rule3(true, 0.4, 8.0, 0.0);



    OnlineEndpointRule rule4(true, 2000.0, std::numeric_limits<BaseFloat>::infinity(), 0.0);
    OnlineEndpointRule rule5(false, 0.0, std::numeric_limits<BaseFloat>::infinity(), 20.0);
	...
	std::string clat_wspecifier = po->GetArg(3);
