...
void SFNnetDecoder::finishDecoding() {
	...
	//clat.Write(//cout);
	string ret = controller->output_string(current_word_syms, clat, &num_frames, &tot_like);
	//cout << "ret '" << ret << "'" << endl;
	...
	if (ret == "!SIL " || ret=="!NOISE ") {
		...
		controller -> my_log("not resetting LM because !SIL or !NOISE");
	}
	else {
		controller -> my_log("resetting LM because not !SIL or !NOISE");
...
void SFNnetDecoder::decode(Vector<BaseFloat>* data) {
	...
	//controller->my_log("nnet listener decoder");
	using namespace fst;
...
//		cerr << "Fresh utterance " << fresh_utterance << endl;
//		cerr << "Lm ready " << lm_ready << endl;
