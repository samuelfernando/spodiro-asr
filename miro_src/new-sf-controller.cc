...
//	cerr << "Making date string" << endl;
	...
	WAV_Writer writer;
	int kSampleFreq = 20000;
	//cerr << "Creating string " << str << " " << str.c_str() << endl;
	Audio_WAV_OpenWriter(&writer, str.c_str(), kSampleFreq, 2);
	active = true;
	bool first = true;
	while (active) { 
		
		read(read_pipe, &should_listen, sizeof(bool));
		int size;
		
		read(read_pipe, &size, sizeof(int));
		if (first) {
			cerr << "Started receiving data in ASR module" << endl;
			first = false;
		}
		//cerr << "size received " << size << endl; 
		int16 buf[size];
		short out[size];
		Vector<BaseFloat> kaldi_data(size/2);
		
		//cerr << "Waiting to read from YARP" << endl;
		int actual_size = read(read_pipe, buf, sizeof(buf));
		//cerr << "actual_size " << actual_size << endl;
		for (int i=0;i<size;++i) {
			//out_file << buf[i] << " " << buf[i+1] << endl;
			out[i] = buf[i]*64;
			//out[i/2] = short((double(buf[i])+double(buf[i+1]))/1024);
		}
		
		for (int i=0;i<size-1;++i) {
			kaldi_data.Data()[i/2] = ((float)(out[i]+out[i+1]))/2;
		}
		Audio_WAV_WriteShorts( &writer, out, 4000);
		hear(&kaldi_data);
