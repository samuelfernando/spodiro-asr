...
train_base=augmented
mfccdir=$exp/mfccs/hires_mfccs
utils/copy_data_dir.sh $train ${train}_hires
  steps/make_mfcc.sh --nj 40 --mfcc-config conf/mfcc_hires.conf \
     --cmd "$train_cmd" ${train}_hires $exp/make_hires/$train_base $mfccdir || exit 1;
 steps/compute_cmvn_stats.sh ${train}_hires $exp/make_hires/$train_base $mfccdir || exit 1;
...
if [ $stage -le 2 ]; then
  ...
  steps/align_fmllr.sh --nj 40 --cmd "$train_cmd" \
    $train $lang $exp/tri4a $exp/nnet2_online/tri4a_ali
fi
...
if [ $stage -le 3 ]; then
  ...
  steps/train_lda_mllt.sh --cmd "$train_cmd" --num-iters 13 \
    ...
    5000 10000 ${train}_hires $lang \
     $exp/nnet2_online/tri4a_ali $exp/nnet2_online/tri5b
fi
#
if [ $stage -le 4 ]; then
...
#
  steps/online/nnet2/train_diag_ubm.sh --cmd "$train_cmd" --nj 30 \
     --num-frames 400000 ${train}_hires 256 $exp/nnet2_online/tri5b $exp/nnet2_online/diag_ubm
fi
#
if [ $stage -le 5 ]; then
#  # even though $nj is just 10, each job uses multiple processes and threads.
  steps/online/nnet2/train_ivector_extractor.sh --cmd "$train_cmd" --nj 10 \
    ${train}_hires $exp/nnet2_online/diag_ubm $exp/nnet2_online/extractor || exit 1;
fi
#
if [ $stage -le 6 ]; then
...
#  # handle per-utterance decoding well (iVector starts at zero).
  steps/online/nnet2/copy_data_dir.sh --utts-per-spk-max 2 ${train}_hires \
   ${train}_hires_max2
#
steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj 30 \
    ${train}_hires_max2 $exp/nnet2_online/extractor $exp/nnet2_online/ivectors_train || exit 1;
fi
#
#if [ $stage -le 7 ]; then
