...
# 
for datadir in head_test; do
##	for subdir in clean 5dB 10dB 20dB; do
  #for datadir in $train; do
  for subdir in clean; do
    steps/online/nnet2/extract_ivectors_online.sh --cmd "$train_cmd" --nj $njobs \
      $data/pf/$datadir/${subdir}_hires $exp/nnet2_online/extractor $exp/nnet2_online/ivectors_pf_${datadir}_${subdir} || touch $exp/nnet2_online/.error &
  done
done
wait
